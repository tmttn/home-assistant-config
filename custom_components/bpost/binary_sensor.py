...
async def async_setup_entry(hass, entry: ConfigEntry, async_add_entities: AddEntitiesCallback):
    ...
    async_add_entities(
        BpostBinarySensor(entry_data.coordinator, sensor_id)
        for sensor_id in entry_data.coordinator.data["binary_sensor"].keys()
    )
    ...
    def add_new_entities() -> None:
        ...
        async_add_entities(BpostBinarySensor(entry_data.coordinator, sensor_id) for sensor_id in to_add)
